...
def make(db_file_name, count_range, structure_scheme, structure_data):
   ...
   for count in count_range:
       ...
       ti = time.time() # запускаем счетчик времени
       bubble_sort(tch_list_sorted) # выполняем сортировку пузырьком
       results['bubble'].append((count, time.time - ti))
       ...
       results['quick'].append((count, time.time(), ti))
       ...
       results['merge'].append((count, time.time() - ti))
       print('\n')

...
"""
Создает словарь: key - факультет, obj - list
:returnn: collections.defaultdict
"""

def multimap_creation(cringe):
    array = cringe.copy()
    for i in range(len(cringe)):
        array[i] = teacher.str_to_list(str(array[i]))
    keys = [array[i][0] for i in range(len(array))]
    keys_array = zip(keys, cringe)
